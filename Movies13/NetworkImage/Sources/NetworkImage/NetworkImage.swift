...
public struct NetworkImage: View {
	...
	public init(url: URL) {
		self.init(imageLoader: NetworkImageLoader(url: url))
	}
	
	public init<R: NetworkDataRequest>(request: R, network: Network) {
		self.init(imageLoader: NetworkImageLoader(request: request, network: network))
	}

	public init(imageLoader: NetworkImageLoader) {
